...
tabDat <- scenLst %>% 
  ...
  rename(scenario1 = 2, scenario2 = 3, scenario3 = 4)

varRow <- sapply(1:nrow(tabDat), function(x) 
  ifelse(all(unlist(tabDat[x,2:nScenarios]) == unlist(tabDat[x,2])), 0, 1))
wVarRow <- which(varRow == 1)
